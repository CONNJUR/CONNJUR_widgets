/*  mgNhsqc.c
    ...
    triax probe).  (MRG added option for inversion recovery 1H relaxation
    measurement.)
    ...
    Written by MRB, December 1997, starting with ghn_co from ProteinPack.
    Revised and improved to a standard format by MRB, BKJ and GG for the 
    ProteinPack, January 1998, so as to include calculation of the above 
   ...
   16. TROSY and COUPLED 2D SPECTRA:
       ...
       change from the normal SE train in the ProteinPack sequences.
       Relaxation time measurements have not yet been implemented for TROSY
       spectra (requires customer demand !).
       
   17. 1H RELAXATION:
       Set inv_rec='y' and array H1_delay.  The percentage inversion will be 
       dependent upon d1 (as well as other factors), but the rate of recovery should 
       be independent of d1.   
...
char        f1180[MAXSTR],   		      /* Flag to start t1 @ halfdwell */
	    ...
	    inv_rec[MAXSTR];                    /* insert 1H relaxation delay */
...
double      tau1,         				         /*  t1 delay */
	    ...
	    H1_delay = getval("H1_delay"),          /* 1H relaxation T1 delay */
...
/* CHECK VALIDITY OF PARAMETER RANGES */
  ...
  { text_error( " gt1 is too small. Make gt1 equal to %f or more.\n",    
    (-2.0e-4 + pwHs + 1.0e-4 + 2.0*POWER_DELAY) );		    abort(1); }
...
if (H1_delay < (gtinv + 152.0e-6 + 20.0e-6))
{ text_error("H1_delay must be greater than %f ! ", gtinv + 152.0e-6 + 20.0e-6);
             abort(1); }
...
/*  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  */
	...
	/* Insert for 1H relaxation inversion pulse and relaxation delay      */
	if (inv_rec[0] == 'y') {
            rgpulse(pw,zero,1.0e-6,0.0);    
            delay(2*tNH);                                 /* delay=1/2J(NH)   */
            sim3pulse(2.0*pw,0.0e-6,2*pwN,zero,zero,zero,0.0,0.0);
            delay(2*tNH);                                 /* delay=1/2J(NH)   */
            rgpulse(pw,zero,0.0,0.0);
	    delay(2.0e-6);
            zgradpulse(gzlvlinv,gtinv);
            delay(150.0e-6);
            delay(H1_delay - gtinv - 152.0e-6 - 20.0e-6);
        }
