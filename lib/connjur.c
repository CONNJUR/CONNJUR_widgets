...
ReqVarianPar extractVarPar(GtkTreeStore *VarianTree)
{
    ...
    valid = gtk_tree_model_get_iter_first(GTK_TREE_MODEL(VarianTree), &iter);
    while (valid){
        gtk_tree_model_get(GTK_TREE_MODEL(VarianTree), &iter, 0, &param, 1, &value, -1);
        ...
        valid = gtk_tree_model_iter_next(GTK_TREE_MODEL(VarianTree), &iter);
    }
    return (parameters);
}
...
int buildSamplingWindow(GtkBuilder *builder, char *path)
{
    ...
    drawArea = GTK_WIDGET(gtk_builder_get_object(builder,"drawingarea2"));
    g_signal_connect (G_OBJECT(drawArea), "draw", G_CALLBACK(sampling_callback), (gpointer) *scheduleContent);
...
gboolean sampling_callback (GtkWidget *widget, cairo_t *cr, gpointer data)
{
    double dot_width = 5, dot_height=5;
    
    point sched[10000];
    int i, num_lines, x_max, y_max;
    int dim = 1;
    char test[] = "0   0   \n1   3   \n2   4   \n10  20  \n5   8   \n32  12  \n4   30  \n";
    char test1D[] = "0  \n  8   \n16   \n32  \n64   \n";
    GScanner *scanner;
    ...
    if (scanner->token != G_TOKEN_INT) return 0;    // bad file
    sched[scanner->line-1].x = (int) scanner->value.v_int;
    x_max = sched[scanner->line-1].x;
    g_scanner_get_next_token (scanner);
    if (scanner->token == G_TOKEN_EOF) return 1;    // 1 int schedule
    if (scanner->token != G_TOKEN_INT) return 0;    // non-integer, bad-file
    if (scanner->line == 1) {                       // 2D schedule
        dim = 2;
        sched[scanner->line-1].y = (int) scanner->value.v_int;
        y_max = sched[scanner->line-1].y;
    } else {
        sched[scanner->line-1].x = (int) scanner->value.v_int; // 1D sched
        sched[scanner->line-1].y = 0;
        sched[scanner->line-2].y = 0;
        y_max = 0;
    }
    num_lines = (int) scanner->line;
    g_scanner_get_next_token (scanner);     // x
    while (scanner->token == G_TOKEN_INT) {
        num_lines = (int) scanner->line;
        sched[scanner->line-1].x = (int) scanner->value.v_int;
        if (sched[scanner->line-1].x > x_max) x_max = sched[scanner->line-1].x;
        if (dim == 1) sched[scanner->line-1].y = 0;
        //printf("x:%d ",(int) scanner->value.v_int);
        g_scanner_get_next_token (scanner); // y if dim == 2, otherwise x
        if (dim == 2) {
            sched[scanner->line-1].y = (int) scanner->value.v_int;
            if (sched[scanner->line-1].y > y_max) y_max = sched[scanner->line-1].y;
            //printf("y:%d \n",(int) scanner->value.v_int);
            g_scanner_get_next_token (scanner); // x
        } //else sched[scanner->line-2].y == 0;
    //    printf("%d %d\n", sched[num_lines-1].x, sched[num_lines-1].y);
    }
    g_scanner_destroy(scanner);
    
//    printf("%d %d", x_max, y_max);
    
    if (x_max < 128 && y_max < 128) {
        
        if (y_max == 0) dot_height=20;
...
void nbx_button_clicked (GtkWidget *widget, gpointer data)
{
    ...
    if (res == GTK_RESPONSE_ACCEPT)
    {
        ...
        // Do something here
        sprintf(cst_command, "/usr/software/bin/spectrum-translator -st varian -dt nbx -sd %s -df %s", (char *)data, output);
